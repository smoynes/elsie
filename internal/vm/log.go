...
func (vm *LC3) LogValue() log.Value {
	return log.GroupValue(
		...
		log.String("DDR", vm.Mem.Devices.DDR().String()),
		log.String("DSR", vm.Mem.Devices.DSR().String()),
		log.String("KBDR", vm.Mem.Devices.KBDR().String()),
		log.String("KBSR", vm.Mem.Devices.KBSR().String()),
		log.Any("INT", vm.INT),
		log.Any("REG", vm.REG),
	)
}
